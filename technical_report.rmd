...
print(gbm.roc)
```
Our boosted tree performs at about the same level as our RF model. Based on computational requirements, we would prefer the RF model between these two, but ultimately prefer glm.

We plot the ROC curves for our models together to determine that an all-predictor logistic regression satisfies our modeling objective best.
```{r}
